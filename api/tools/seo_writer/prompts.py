...
- 正文里出现的**每一个数字和专有名称，都必须来自上面这份清单**，写法逐字一致。
...
- 清单里没有的数字，**不许写**。宁可把话说定性（"多数"、"少量"），也不许编一个数字。
...
上面那份参考资料里有工具名、金额、百分比、期限这些**可验证的具体信息**。
把它们用起来 —— 每个 H2 至少落地一个：一个带单位的数字（$/%/天/KB/ms）、
一个真实的工具或功能名称、或一个具体的门槛条件。

- **只能用参考资料里出现过的事实。资料里没有的数字、比例、排名、"超过 X 家品牌"
  这类统计，一个都不许编。** 资料没给数字的段落，就用工具名或具体步骤代替，
  或者干脆把话说小。宁可少写一个数字，也不许造一个。
