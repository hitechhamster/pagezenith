...
# 外链自动选取
# ============================================================
LINKS_PROMPT = """你是 SEO 外链策略专家。从下方的文章列表中,选出 **恰好 2 篇** 最适合作为本文外链的文章。
...
## 选择标准
1. 与本文主题高度相关
2. 起到补充/延伸作用,不是重复本文内容
3. 两个链接应分散在不同章节
4. 锚文本必须用 {language} 书写,自然融入句子
...
{{
  "links": [
    {{
      "url": "完整 URL",
      "title": "原文标题",
      "anchor_text": "用 {language} 写的自然锚文本短语(5-15 个词)",
      "suggested_section": "建议插入到大纲的哪个章节(写 H2 标题)"
    }},
    {{
      "url": "完整 URL",
      "title": "原文标题",
...
本文需要插入以下 {n} 个外链,以 Markdown 链接格式 `[锚文本](URL)` 自然融入正文:
{links_detail}

### 外链插入规则
1. 锚文本必须自然融入上下文,读起来像正常句子的一部分
2. 两个外链分散在不同章节
3. 不要单独用一句话介绍链接,在正常论述中自然带出
"""
