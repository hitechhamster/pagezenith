...
# 信息增益的结构来源不能靠模型凭空发明「竞品缺口」。只用搜索页实际出现的
# PAA / 相关搜索，先做意图过滤，再各搜一层补素材。这样新内容仍围绕主任务，
# 也能明确追溯到读者真实在搜的关联需求。
COVERAGE_QUERIES_BLOCK = """

**已验证并补搜过素材的关联需求：**
{items}

这些是主关键词读者自然会继续搜索的问题，不是要求你逐条写成 H2。
对每一项先判断：回答它是否能帮助读者完成本文的核心任务；只有能帮助的才采用。
**优先把它融入最相关的主线章节**，只有它本身是独立且必要的决策步骤时才单开 H2。
每个采用的点都要比竞品多给一个具体判断、机制、条件或可执行步骤；不要为了显得独特
另起旁支。主线章节应占文章主体，关联需求内容合计通常不超过约三成。"""

# 公开版沿用原有的「竞品缺口角度」编排；只在内部 BYOK 试行上面的严格关联需求模式。
...
{items}
从中选 1–2 个写进大纲，每个起一个 H2 —— 这是本文比竞品多给的东西。"""
...
3.  **体现E-E-A-T (Experience, Expertise, Authoritativeness, Trustworthiness):**
    * **经验 (Experience - E):** ⚠️ 写手**没有第一手经验**，要求它"融入生动的亲身经历"只会得到
      ...
      不同情况下的取舍条件、容易踩的坑 —— 把这些写进该节的「讲什么」里。
      只有当参考资料里存在**真实且可指名的例子**时，才规划案例，并写明出处。
    * **专业性 (Expertise - E):** 规划展示对主题**深度理解和专业知识**的部分，例如：深入的原理解释、对复杂概念的清晰拆解、专业的对比分析、行业背景或趋势解读等。
      **深度原理补充**：选最相关的**一个** H2，在该节的「讲什么」里安排一小段（2–3 句）
      底层原理或学科知识 —— 比如写 ebike 电池时讲三元锂与磷酸铁锂的化学结构差异，
      写温度传感器时讲热电偶的 Seebeck 效应。这种内容来自教材和论文、搜索结果里没有，
      读者可能看不全懂但会觉得"这个作者真懂行"。**只选一个节、只写一小段，不要到处加。**
      不许编造论文引用或具体实验数据——只讲定性的原理和机制。
    * **权威性 (Authoritativeness - A):**
        * **关键事实与数据支撑：** 对于涉及事实、数据、统计的论点，在相应大纲节点下，列出该节要用的**核心事实或数据要点**（只能从上面的事实清单和参考内容里取，逐条写原文）。
    * **可信度 (Trustworthiness - T):**
        ...
        * **信息呈现优化：** 如果某节有多维数据对比，可以在「结构化块」里标「表」；操作流程用步骤列表；简单罗列用项目列表。不是每节都需要结构化块——信息本身不复杂时，散文比列表更好读。
...
3.  **内容大纲 (Outline)** —— 严格用下面的格式，每节只有三样东西，除此之外什么都不写：
    ```
    # H1 标题（{language}）
    - 导语：2–4 句，说清这篇解决什么问题、给谁看（不设标题）
    ## H2 标题（{language}）
    - 讲什么：2–4 条要点，中文，一条一句，只说内容不说写法
    - 用哪些事实：从上面事实清单 / 参考内容里挑，逐条写原文；没有就写「无」
    - 结构化块：适合用表 / 清单 / 步骤表的写明形式和列头；不适合写「无」
    ### H3 标题（如需要，{language}）
    ```
    **禁止出现**：写作指令、E-E-A-T 提示、Purpose / Value、预估字数、重要性标注、人称要求、
    关键词次数、任何方括号标注。这些是说给写手听的话，会原样泄漏进正文
    （实测正文里出现过「使用 We 的专业视角」变成的 "We control…"），大纲里一个字都不许有。
...
适当使用富文本格式（列表、粗体、斜体）让结构清晰，避免多段纯文字摞在一起。
哪一节的信息适合用清单 / 步骤表摆，就在「结构化块」里写明形式；不适合写「无」，不要硬配。
**表格不是必需的。** 只在多维数据对比确实需要表格时才标「表」，整篇不超过 3 张；大多数节用列表或散文更合适。
注意，不要设计任何内部或者外部链接！
注意，不要设计任何内部或者外部链接！
注意，不要设计任何内部或者外部链接！
注意，不要设计任何内部或者外部链接！
...
---
**输出语言：「讲什么 / 用哪些事实 / 结构化块」的说明**全部用简体中文**（这份大纲是给
...
# 大纲中的第 2 节随运行模式切换：线上不变，BYOK 才试行严格关联需求模式。
OUTLINE_ORIGINALITY_LEGACY = """2.  **原创性 & 显著附加价值 (Originality & Significant Added Value):**
...
OUTLINE_ORIGINALITY_GROUNDED = """2.  **信息增益 & 显著附加价值 (Information Gain & Significant Added Value):**
    * **围绕关联需求加深，而非另造话题：** 上面「搜索页实况」若列了「已验证并补搜过素材的关联需求」，只能从中选择能推进本文核心任务的内容增益。优先将其融入最相关的既有 H2；**不要要求固定数量的独特章节，也不要为了差异化凭空新起一个 H2。**
      只有某个关联需求本身是读者完成核心任务必经的独立决策步骤时，才单开 H2。采用时，在该小节说明中标注「【关联需求增益：对应搜索词】」，并给出竞品欠缺的**具体判断、机制、适用条件、取舍标准或可执行步骤**。文章主体必须持续服务主关键词；关联需求内容总量通常不超过约三成。
      ⚠️ **不要规划「第一手经验分享」「我们的客户案例」这类需要亲身经历的内容** ——
      写手没有经历，只会编（实测七篇产出全部编出了假案例或假经验句）。
    * **避免同质化：** 除非能提供显著不同的解释、更深层次的洞察或更新的信息，否则应避免重复参考内容中已经泛滥且无新意的信息点。"""

# Reddit 真实讨论上下文（可选）：BYOK 用来验证关联需求，线上维持原有独特价值编排。
REDDIT_CONTEXT_GROUNDED = """
## 社媒真实讨论（Reddit，用于挖未被满足的需求）
{reddit}
**使用说明：** 这是真实用户的原话。请从中识别 **高频痛点、被反复问但没被好好回答的问题、以及现有内容普遍忽略的角度**，
只在它们能帮助读者完成本文核心任务时采用；优先融入相应主线章节，不要自动单开章节。
采用时标注「【关联需求增益：Reddit 痛点】」。严禁直接复制原话。
"""

REDDIT_CONTEXT_LEGACY = """
## 社媒真实讨论（Reddit，用于挖未被满足的需求）
{reddit}
**使用说明：** 这是真实用户的原话。请从中识别 **高频痛点、被反复问但没被好好回答的问题、以及现有内容普遍忽略的角度**，
...
2. 保持用户没有提出异议的部分
3. 保持原大纲的格式：每节只有「讲什么 / 用哪些事实 / 结构化块」三样，
   不写写作指令、E-E-A-T 提示、Purpose / Value、预估字数、方括号标注
4. 标题里不要加年份
...
信息密度是文章质量的核心。
一条具体信息是：一个带单位的设定值、一个真实的工具或产品型号、一个标准号、
一个判断门槛、一种失败的具体表现、一条带出处的数据。同一个值说三遍只算一条。
**表格不是必需的** —— 只在多维数据对比确实需要时才建，整篇不超过 3 张。其余用列表或散文。
**事实清单里标了「竞品没写」的条目优先写进去** —— 那是这篇比排在前面的页面多给的东西，
也是它有资格被收录的理由。
...
- **两类数字，规矩不一样。这是本文能不能比竞品多给东西的关键。**
  ...
  · **建议型（读者该把它设成多少）**：上传尺寸、压缩阈值、每页条数、超时秒数、
    目标指标值、版本号、设置项名称。**不许编文件名、表格名、模板名。**
...
输出Markdown格式。操作流程和机械原理用有序步骤列表说明，不要用 +、-、|、箭头或框线字符
拼接 ASCII/Unicode 流程图，不要输出 Mermaid 图。比较数据用标准 Markdown 表格。
只有真正的程序代码才用围栏代码块；不要把整篇文章包进代码块。
...
不要新增 ASCII/Unicode 字符流程图或 Mermaid。若原文有字符拼图，将其改写为有序步骤列表，
逐项保留步骤、顺序、分支条件和说明；这项格式转换优先于下文的原样保留要求。
真正的程序代码及其围栏保持原样，不要把整篇文章包进代码块。

{readability_note}

只做两件事：
1. 润色到上面的目标阅读年级 —— 拆长句、换掉绕的词。
2. 长短句交错 —— 短句制造停顿，长句承载因果和条件，别通篇一个长度。

不做别的：每个 markdown 标题、表格、列表、链接、反引号片段原样保留；
数字、参数、名称、事实原样搬过去，不加不删；篇幅与原文相当；
关键词 "{main_keyword}" 和 "{secondary_keyword}" 保留，正文里的专有名词保持原文大小写。
...
POLISH_LIGHT_PROMPT = """**Task: Lightly rewrite the following text in {language}.**

不要新增 ASCII/Unicode 字符流程图或 Mermaid。若原文有字符拼图，将其改写为有序步骤列表，
逐项保留步骤、顺序、分支条件和说明；这项格式转换优先于下文的原样保留要求。
真正的程序代码及其围栏保持原样，不要把整篇文章包进代码块。

这篇的阅读年级已经达标，不要整篇简化。只做一件事：**长短句交错** ——
短句制造停顿，长句承载因果和条件，别通篇一个长度。

其余原样保留：每个 markdown 标题、表格、列表、链接、反引号片段；数字、参数、名称、事实不加不删；
篇幅与原文相当；关键词 "{main_keyword}" 和 "{secondary_keyword}" 保留。
...
# SEO 元数据
# ============================================================
# 时间概念：模型的预训练截止在过去，不告诉它今天几号就会把 2024 当"最新"写进标题。
# 2026-09-05 实测 ebike 那篇 H1 带 "(2024)"。拼在大纲 / 正文 prompt 最前面。
DATE_LINE = """（背景：今天是 {today}。标题里不要加年份；引用数据时写数据本身的年份，不要把往年的数当成现状。）
...
# --------------------------------------------------------------------------- #
# 搜索页关联需求的意图过滤（2026-09-08 用户报：找代工厂的文章里写进了"哪个牌子最好"）
# --------------------------------------------------------------------------- #
# 原来只有 density_audit._on_topic 那道词面过滤：把问题里主关键词之外的实词拎出来，
# 看有没有出现在竞品标题里。实测它两个方向都错 ——
...
# 因为那个判据跟**搜索意图**无关，只跟"某个词碰巧在不在标题里"有关。
# factory（B2B 找代工）和 brand（B2C 挑品牌）共享全部修饰词，词面上分不开，只能问模型。
QUESTION_FILTER_PROMPT = """下面是搜索结果页上真实出现的关联需求（People Also Ask 或相关搜索）。
判断每一项和本文是不是**同一个搜索意图，或是完成主任务时自然需要解决的下一步**。
...
判断标准：
- 搜主关键词的人，想解决的是同一件事或同一任务的必要下一步 → 保留
- **词面很像但要解决的事不同 → 丢弃**。典型例子：主关键词是「找代工厂」（B2B 采购，
  读者是要下单的买家），而问题问的是「哪个牌子最好」（B2C 选购，读者是要买来自己用的）。
  修饰词完全一样，意图完全不同，答它等于写了一节跑题的内容。
- 拿不准 → 保留。漏掉一个相关问题只是少写一点，答一个跑题问题会毁掉整节。

只输出保留下来的条目，一行一个，**原文照抄**，不要编号、不要引号、不要解释。
一个都不该保留就输出 NONE。

关联需求：
{questions}"""

QUESTION_FILTER_PROMPT_LEGACY = """下面是搜索结果页上「People Also Ask」里的问题。
判断每个问题和本文是不是**同一个搜索意图**。
...
判断标准：
- 搜主关键词的人，想解决的是同一件事 → 保留
- **词面很像但要解决的事不同 → 丢弃**。典型例子：主关键词是「找代工厂」（B2B 采购，
  读者是要下单的买家），而问题问的是「哪个牌子最好」（B2C 选购，读者是要买来自己用的）。
  修饰词完全一样，意图完全不同，答它等于写了一节跑题的内容。
- 拿不准 → 保留。漏掉一个相关问题只是少写一点，答一个跑题问题会毁掉整节。

只输出保留下来的问题，一行一个，**原文照抄**，不要编号、不要引号、不要解释。
一个都不该保留就输出 NONE。
