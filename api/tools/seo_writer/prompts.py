...
3.  **体现E-E-A-T (Experience, Expertise, Authoritativeness, Trustworthiness):**
    ...
    * **可信度 (Trustworthiness - T):**
        ...
        * **信息呈现优化（图表/列表）：** 识别大纲中至少一个适合通过**图表、表格、项目列表或步骤列表**等可视化或结构化形式呈现复杂信息、对比数据或操作流程的部分，写进该节的「结构化块」里。
...
你非常喜欢列出数据和事实，非常在乎文章的信息密度。
哪一节的信息适合用表 / 清单 / 步骤表摆，就在「结构化块」里写明形式和列头；不适合的写「无」，不要硬配。**整篇最多 3 张表**：只给最值得用表的三节标「表」，其余标「清单」或「无」。
注意，不要设计任何内部或者外部链接！
注意，不要设计任何内部或者外部链接！
注意，不要设计任何内部或者外部链接！
注意，不要设计任何内部或者外部链接！
...
数据适合用表格 / 清单摆的就摆，不适合的别硬建。**整篇最多 3 张表**，其余用列表或散文。
