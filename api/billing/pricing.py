...
PRICES: dict[tuple[str, str], dict[str, int]] = {
    ...
    # ⚠️ "整篇成本 ¥0.80" 是早期单次测算，**已被推翻**：2026-09-04 按 20 条真实流水
    # 重算，均值是 **¥1.18**（大纲 ¥0.358 + 正文 ¥0.743 + 润色 ¥0.078），偏乐观 47%。
    # 正文那步波动很大（¥0.29–1.36，跟字数走），单次测算不可靠，要看均值。
    # 9 点毛利 81.5%（大卡口径）—— 仍是全站毛利最低的一条，别再往下压。
    ("seo-writer", "polish"):   {"pro": 2},     # 正文写完自动跑（不再是可选按钮）
    ("seo-writer", "image"):    {"pro": 3},     # 每张配图（高质量档，见下方成本）

    # ── 五个非写作工具：2026-09-04 按实测成本重定价 ──────────────────
    # 背景：这几个价原本全是拍的（这一段以前连注释都没有）。当天给每个工具真跑一次、
    # 读 usage.est_cost，发现**定价倒挂**：旗舰的写作线毛利 81.5%，这五个全在 95–100%。
    # 摊到每一点看：写作每点成本 ¥0.131，内容差距只有 ¥0.019 —— 差 6.7 倍。
    # 内容差距的真实成本只有一篇文章的 1/12，却收着一篇文章 5/9 的价，最离谱，从 5 降到 3。
    #
    #   工具            实测成本    旧价 → 新价   新毛利率(大卡口径)
    #   内容差距        ¥0.097      5 → 3        95.4%
    #   外链拓客        ¥0.065      2 → 2        95.4%   ← 非写作里成本最高，不动
    #   Reddit 选题     ¥0.025      2 → 1        96.4%
    #   文章质检        ¥0.018      1 → 1        97.4%
    #   站点侦察        ¥0.000      2 → 1        ~100%   ← 全程不调 LLM，只有抓取+规则
    #
    # ⚠️ 两条局限，改价前必读：
    #   ① 上面每个数只跑了 **1 次**，成本随输入波动（英文长竞品页会更贵）。有真实
    #      流量后要照同样方法重测：跑一轮 → 查 usage.est_cost → 回填这里。
    #   ② est_cost **只统计 LLM token**，不含 SERP 调用（内容差距每次 2 次 ≈ ¥0.015）
    #      和抓取带宽。绝对值小，但别把它当成全成本。
    ("seo-gap", "analyze"):     {"pro": 3},
    ("article-quality", "check"): {"pro": 1},
    ("reddit-research", "run"): {"pro": 1},
    ("outreach", "run"):        {"pro": 2},
